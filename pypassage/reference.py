...
def book_name(bible_data, book_n, abbreviated=False, single_psalm=False):
    ...
    if abbreviated:
        return bible_data.standard_book_names[book_n][2]
    else:
        if single_psalm:
            return "Psalm"
        else:
            return bible_data.standard_book_names[book_n][1]
...
class MCBGroup(object):
    ...
    def reference_string(self, abbreviated, dash):
        ...
        if abbreviated:
            book = bibledata.standard_book_names[self.start_book_n][2]
        else:
            book = bibledata.standard_book_names[self.start_book_n][1]
