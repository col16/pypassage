from pypassage.reference import PassageCollection as C
from pypassage.reference import Passage as P
from pypassage.reference import PassageDelta as D
from pypassage.reference import InvalidPassageException
from pypassage.reference import passages_from_string
...
class TestBookData(unittest.TestCase):
        ...
        for (book_number, chapters) in list(bd.number_chapters.items()):
            for chapter in range(1, chapters + 1):
                if bd.last_verses.get((book_number, chapter), None) == None:
                    self.fail("Could not find last_verses for book " + \
                              str(book_number) + " and chapter " + str(chapter))
            if bd.last_verses.get((book_number, chapters + 1), None) != None:
                self.fail("last_verses found for book " + str(book_number) + \
                          " and chapter " + str(chapter) + ", when this book was only " + \
                          "supposed to have " + str(chapters) + " chapters")
        for ((book_number, chapter), verses) in list(bd.missing_verses.items()):
            for verse in verses:
                if verse >= bd.last_verses[book_number, chapter]: self.fail(
                    "missing verse greater than or equal to corresponding " + \
                    "last verse")
    ...
    def test_number_chapters_in_bible_B(self):
        self.assertEqual(sum([chapters for (book, chapters) in list(
            bd.number_chapters.items())]), 1189)

    def test_sum_of_last_verses(self):
        self.assertEqual(sum([verses for (book_chapter, verses) in list(
            bd.last_verses.items())]), 31103)

    def test_number_missing_verses(self):
        self.assertEqual(sum([len(x) for (book_chapter, x) in list(
            bd.missing_verses.items())]), 17)
    ...
    def test_abbreviations(self):
        ...
        for book_n in range(1, 67):
            ...
            if abbr_3 == None:
                ...
                self.fail("book_number entry for " + str(abbr_3) + " not the " + \
                          "same as its number in book_names dict")
            elif abbr_3 in found_3:
                self.fail("Duplicate found for three-letter code " + str(abbr_3))
            elif abbr_std in found_std:
                self.fail("Duplicate found for standard abbreviation " + \
                          str(abbr_std))
...
class TestPassage(unittest.TestCase):
    ...
    def test_normal_function(self):
        self.assertEqual(P(book='GEN', start_chapter=1, start_verse=1,
                           end_chapter=2, end_verse=1).is_valid(), True)
        self.assertEqual(P(book='genesis', start_chapter=1, start_verse=1,
                           end_chapter=2, end_verse=1).is_valid(), True)
        self.assertEqual(P(book='GEN', start_chapter=1, start_verse=1,
                           end_chapter=2, end_verse=1), P(1, 1, 1, 2, 1))
        self.assertEqual(P(book='GEN', start_chapter=1, start_verse=1,
                           end_book='REV', end_chapter=1, end_verse=1).is_valid(), True)
        self.assertEqual(P(book='GEN', end_book='LEV').is_valid(), True)

    # Testing passage normalisation
    def test_multi_chapter_books(self):
        self.assertEqual(P(book='GEN', start_chapter=2, start_verse=6,
                           end_chapter=3, end_verse=5).is_valid(), True)
        self.assertEqual(P(book='GEN', start_chapter=2, start_verse=6,
                           end_chapter=3), P('Gen', 2, 6, 3, 24))
        self.assertEqual(P(book='GEN', start_chapter=2, start_verse=6,
                           end_verse=7), P('Gen', 2, 6, 2, 7))
        self.assertEqual(P(book='GEN', start_chapter=2, end_chapter=3,
                           end_verse=5), P('Gen', 2, 1, 3, 5))
        self.assertEqual(P(book='GEN', start_verse=6, end_chapter=3,
                           end_verse=5), P('Gen', 1, 6, 3, 5))
        self.assertEqual(P(book='GEN', start_chapter=2, start_verse=6),
                         P('Gen', 2, 6, 2, 6))
        self.assertEqual(P(book='GEN', start_chapter=2, end_chapter=3),
                         P('Gen', 2, 1, 3, 24))
        self.assertEqual(P(book='GEN', start_chapter=2, end_verse=5),
                         P('Gen', 2, 1, 2, 5))
        self.assertEqual(P(book='GEN', start_verse=6, end_chapter=3),
                         P('Gen', 1, 6, 3, 24))
        self.assertEqual(P(book='GEN', start_verse=6, end_verse=7),
                         P('Gen', 1, 6, 1, 7))
        self.assertEqual(P(book='GEN', end_chapter=3, end_verse=5),
                         P('Gen', 1, 1, 3, 5))
    ...
    def test_single_chapter_books(self):
        self.assertEqual(P(book='PHM', start_chapter=1, start_verse=5,
                           end_chapter=1, end_verse=6), P(book='PHM', start_verse=5,
                                                          end_verse=6))
        self.assertEqual(P(book='PHM', start_chapter=1, start_verse=5,
                           end_verse=6), P(book='PHM', start_verse=5, end_verse=6))
        self.assertEqual(P(book='PHM', start_verse=5, end_chapter=1,
                           end_verse=6), P(book='PHM', start_verse=5, end_verse=6))
        self.assertEqual(P(book='PHM', start_verse=5, end_verse=6).is_valid(),
                         True)
        self.assertEqual(P(book='PHM', start_chapter=2, end_chapter=3),
                         P(book='PHM', start_verse=2, end_verse=3))
        self.assertEqual(P(book='PHM', start_chapter=1, start_verse=6),
                         P(book='PHM', start_verse=1, end_verse=6))
        self.assertEqual(P(book='PHM', start_chapter=3, start_verse=6),
                         P(book='PHM', start_verse=3, end_verse=6))
        self.assertEqual(P(book='PHM', start_chapter=3), P(book='PHM',
                                                           start_verse=3, end_verse=3))
        self.assertEqual(P(book='PHM', start_verse=6), P(book='PHM',
                                                         start_verse=6, end_verse=6))
        self.assertEqual(P(book='PHM', end_verse=9), P(book='PHM',
                                                       start_verse=1, end_verse=9))
        self.assertEqual(P(book='PHM'), P(book='PHM', start_verse=1,
                                          end_verse=25))
    ...
    def test_fixable(self):
        self.assertEqual(P('Gen', 1, 1, 51, 1), P('Gen', 1, 1, 50, 26))  # end_chapter is
        # past end of book
        self.assertEqual(P('Gen', 1, 1, 2, 26), P('Gen', 1, 1, 2, 25))  # end_verse is

    # greater than last verse for that chapter
    ...
    def test_unfixable(self):
        ...
        try:
            ...
            pass
        try:
            P('Gen', 1, 32, 2, 1)  # start_verse is greater than last verse for that
            # chapter
    ...
    def test_passage_strings(self):
        self.assertEqual(str(P(book='GEN', start_chapter=1, start_verse=1,
                               end_chapter=1, end_verse=2)), "Genesis 1:1-2")
        self.assertEqual(str(P(book='GEN', start_chapter=1, start_verse=1,
                               end_chapter=2, end_verse=1)), "Genesis 1:1-2:1")
        self.assertEqual(str(P(book='GEN', start_chapter=1, start_verse=3,
                               end_chapter=2, end_verse=25)), "Genesis 1:3-2:25")
        self.assertEqual(str(P(book='GEN', start_chapter=1, start_verse=1,
                               end_chapter=1, end_verse=31)), "Genesis 1")
        self.assertEqual(str(P(book='GEN', start_chapter=1, start_verse=1,
                               end_chapter=5, end_verse=32)), "Genesis 1-5")
        self.assertEqual(str(P(book='GEN', start_chapter=1, end_chapter=2,
                               end_verse=1)), "Genesis 1:1-2:1")
        self.assertEqual(str(P(book='GEN', start_chapter=1, start_verse=5,
                               end_chapter=2)), "Genesis 1:5-2:25")
        self.assertEqual(str(P(book='GEN', start_chapter=1, start_verse=1,
                               end_chapter=50, end_verse=26)), "Genesis")
        self.assertEqual(str(P(book='PHM', start_verse=1, end_verse=5)),
                         "Philemon 1-5")
        # Abbreviated version
        self.assertEqual(P(book='GEN', start_chapter=1, start_verse=1,
                           end_chapter=1, end_verse=2).abbr(), "Gn 1:1-2")
    ...
    def test_osis_passage_strings(self):
        ...
        self.assertEqual(P('GEN', 1, 1, 1, 2, 'Exo').osis_reference(),
                         "Gen.1.1-Exod.1.2")
    ...
    def test_number_verses(self):
        ...
        self.assertEqual(P('1Jo', 5, 20, 1, 2, '3Jo').number_verses(per_book=True),
                         {62: 2, 63: 13, 64: 2})
    ...
    def test_proportion_of_book(self):
        self.assertEqual(P(book='1JO', start_chapter=1, start_verse=1,
                           end_chapter=2, end_verse=1).proportion_of_book(), 11.0 / 105.0)

    # Testing verses that don't exist
    def test_verse_that_doesnt_exist(self):
        try:
            P(book='MAT', start_chapter=12, start_verse=47, end_chapter=12,
              end_verse=47)
    ...
    def test_automatic_shortening(self):
        self.assertEqual(P(book='MAT', start_chapter=12, start_verse=47,
                           end_chapter=17, end_verse=21), P(book='MAT', start_chapter=12,
...
class TestPassageCollection(unittest.TestCase):
    ...
    def test_string(self):
        ...
        self.assertEqual(str(C(P('Eph', 1, 1), P('Eph', 1, 3), P('Eph', 1, 5))),
                         "Ephesians 1:1, 1:3, 1:5")
        self.assertEqual(str(C(P('Eph', 1, 9), P('Eph', 1, 3), P('Eph', 1, 5))),
        ...
        self.assertEqual(str(C(P('Eph', 1, 1), P('Eph', 1, 3, 1, 7), P('Eph', 1, 15))),
        ...
        self.assertEqual(str(C(P('Eph', 1), P('Eph', 3), P('Eph', 5, 9))),
                         "Ephesians 1, 3, 5:9")
        self.assertEqual(str(C(P('Eph', 1), P('Eph', 3, 1, 4, 9), P('Eph', 3, 5))),
        ...
        self.assertEqual(str(C(P('Eph', 1), P('Eph', 3, 1, 4, 32), P('Eph', 5, 5),
                               P('Eph', 6))), "Ephesians 1, 3-4, 5:5, 6:1-24")
        self.assertEqual(str(C(P('Eph', 1), P('Eph', 3, 1, 3, 9), P('Eph', 5))),
        ...
        self.assertEqual(str(C(P('Eph'), P('Mat', 5))),
                         "Ephesians; Matthew 5")
        # Consecutive passages from same book
        self.assertEqual(str(C(P('Eph', 1), P('Gen', 1), P('Gen', 3), P('Gen', 5),
                               P('Mat', 5))), "Ephesians 1; Genesis 1, 3, 5; Matthew 5")
        self.assertEqual(str(C(P('Eph', 1), P('Gen', 1, 1), P('Gen', 1, 3),
                               P('Gen', 1, 5), P('Mat', 5), P('Mat', 9), P('Mat', 1))),
                         "Ephesians 1; Genesis 1:1, 1:3, 1:5; Matthew 5, 9, 1")
        self.assertEqual(str(C(P('Eph', 1), P('Eph', 3), P('Eph', 5, 9), P('Mat', 5))),
                         "Ephesians 1, 3, 5:9; Matthew 5")
        self.assertEqual(str(C(P('Eph', 1), P('Eph', 3), P('Eph', 5, 9, 6, 2),
                               P('Mat', 5))), "Ephesians 1, 3, 5:9-6:2; Matthew 5")
        self.assertEqual(str(C(P('Eph', start_chapter=1, end_chapter=3),
                               P('Eph', 4))), "Ephesians 1-3, 4")
        # As soon as a verse is mentioned, the following references must all be
        # chapter and verse
        self.assertEqual(str(C(P('Eph', 1, 1), P('Eph', 3), P('Eph', 5, 9),
                               P('Mat', 5))), "Ephesians 1:1, 3:1-21, 5:9; Matthew 5")
        self.assertEqual(str(C(P('Eph', 1), P('Gen', 3, 2), P('Gen', 3, 6),
                               P('Gen', 8), P('Mat', 5))),
                         "Ephesians 1; Genesis 3:2, 3:6, 8:1-22; Matthew 5")
        self.assertEqual(str(C(P('Eph', 1, 1), P('Eph', 3), P('Eph', 5), P('Mat', 5))),
                         "Ephesians 1:1, 3:1-21, 5:1-33; Matthew 5")

        # E. Single-chapter books
        self.assertEqual(str(C(P('Phm', 1), P('Phm', 1, 3, 1, 6), P('Phm', 15))),
                         "Philemon 1, 3-6, 15")
        self.assertEqual(str(C(P('Mat', 1), P('Phm', 1, 3, 1, 6), P('Phm', 15),
                               P('Rev'))), "Matthew 1; Philemon 3-6, 15; Revelation")
        ...
        self.assertEqual(str(C(P('Gen', 1), P('Gen', 2), P('Gen', 3, 1, 1, 22, 'Exo'))),
                         "Genesis 1, 2; Genesis 3-Exodus 1")
        self.assertEqual(str(C(P('Gen', 1, 1), P('Gen', 1, 2),
                               P('Gen', 1, 3, 1, 2, 'Exo'))),
        ...
        self.assertEqual(C(P('Eph', 1), P('Gen', 3, 2), P('Gen', 3, 6), P('Gen', 8),
...
class TestPassageDelta(unittest.TestCase):
    ...
    def test_delta_chapter_with_passage_end(self):
        ...
        self.assertEqual(P('Gen', 3, 1, 3, 24) + D(chapters=50),
                         P('Gen', 3, 1, 3, 22, 'Exo'))
    ...
    def test_negative_delta_chapter_with_passage_end(self):
        ...
        self.assertEqual(P('Gen', 1, 1, 10, 1, 'Exo') + D(chapters=-10),
                         P('Gen', 1, 1, 50, 1))
        self.assertEqual(P('Gen', 1, 1, 10, 1, 'Exo') + D(chapters=-20),
                         P('Gen', 1, 1, 40, 1))
    ...
    def test_delta_chapter_with_passage_start(self):
        ...
        self.assertEqual(P('Gen', 2, 1) + D(chapters=1, passage_start=True),
                         P('Gen', 1, 1, 2, 1))
        # Adding more chapters than are available
        self.assertEqual(P('Gen', 2, 1) + D(chapters=3, passage_start=True),
                         P('Gen', 1, 1, 2, 1))
        # Delta chapter pushing reference out into previous book
        self.assertEqual(P('Exo', 2, 1) + D(chapters=3, passage_start=True),
                         P('Gen', 49, 1, 2, 1, 'Exo'))
        # Truncation of start_verse
        self.assertEqual(P('Gen', 4, 26) + D(chapters=1, passage_start=True),
                         P('Gen', 3, 24, 4, 26))

    def test_negative_delta_chapter_with_passage_start(self):
        # REMOVING chapters from the start of a passage
        self.assertEqual(P('Gen', 4, 1, 5, 32) + D(chapters=-1, passage_start=True),
                         P('Gen', 5, 1, 5, 32))
        # Delta chapter pushing reference back into next book
        self.assertEqual(P('Gen', 1, 1, 40, 38, 'Exo') + D(chapters=-50,
                                                           passage_start=True), P('Exo', 1, 1, 40, 38))
        # Delta chapter greater than length of passage
        try:
            ...
            pass
        # Truncation of start_verse
        self.assertEqual(P('Gen', 1, 31, 5, 32) + D(chapters=-1, passage_start=True),
                         P('Gen', 2, 25, 5, 32))
    ...
    def test_delta_verse_with_passage_start(self):
        ...
        self.assertEqual(P('Gen', 1, 2) + D(verses=1, passage_start=True),
                         P('Gen', 1, 1, 1, 2))
        # Back into previous chapter
        self.assertEqual(P('Gen', 2, 1) + D(verses=1, passage_start=True),
                         P('Gen', 1, 31, 2, 1))
        self.assertEqual(P('Gen', 2, 1) + D(verses=31, passage_start=True),
                         P('Gen', 1, 1, 2, 1))
        # Back into previous book
        self.assertEqual(P('Exo', 1, 1) + D(verses=1, passage_start=True),
                         P('Gen', 50, 26, 1, 1, 'Exo'))
        # Back to start of bible
        self.assertEqual(P('Gen', 2, 1) + D(verses=100, passage_start=True),
                         P('Gen', 1, 1, 2, 1))

    def test_negative_delta_verse_with_passage_start(self):
        # Removing verses from the start of a passage
        self.assertEqual(P('Gen', 1, 1, 1, 31) + D(verses=-1, passage_start=True),
                         P('Gen', 1, 2, 1, 31))
        # Shorter by a chapter
        self.assertEqual(P('Gen', 1, 31, 2, 1) + D(verses=-1, passage_start=True),
                         P('Gen', 2, 1))
        # Shorter by a book
        self.assertEqual(P('Gen', 50, 26, 1, 1, 'Exo') + D(verses=-1,
                                                           passage_start=True), P('Exo', 1, 1))
...
class TestPassageLookup(unittest.TestCase):
    def test_esv(self):
        self.assertEqual(P('Gen', 1, 1).text(api_key=ESV_API_KEY)[0],
    ...
    def test_cache(self):
        ...
        book_limits = dict([(k, v * 0.5) for (k, v) in list(
            bd.number_verses_in_book.items())])
        ...
        (p, t) = P('Genesis', 27, 40, 34, 28).text(api_key=ESV_API_KEY, cache=sc)
        self.assertEqual(len(sc.cache), 2)
        self.assertEqual(t, False)
...
#        self.assertEqual(passages_from_string("Gen 1:1b"), C(P('Gen',1,1)) )
        self.assertEqual(passages_from_string("Gen 1:1-3"), P('Gen', 1, 1, 1, 3))
#        self.assertEqual(passages_from_string("Gen 1:1-2:4"), C(P('Gen',1,1,2,4)) )
        self.assertEqual(passages_from_string("Gen 1-2"), P('Gen',start_chapter=1,end_chapter=2) )
...
#        self.assertEqual(passages_from_string("Gen1.1,3.2"), C(P('Gen',1,1),P('Gen',3,2)) )
        self.assertEqual(passages_from_string("2 Tm 1:1"), P('2 Tm', 1, 1)) #short abbreviation
        self.assertEqual(passages_from_string("Song of Solomon 1:1"), P('Song of Solomon', 1, 1)) #spaces in book name
